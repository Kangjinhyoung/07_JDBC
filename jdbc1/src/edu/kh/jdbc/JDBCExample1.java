...
public class JDBCExample1 {
	public static void main(String[] args) {
		 ...
		 * - Java 에서 DB에 연결(접근) 할 수 있게 해주는 Java API(Java 에서 제공하는 코드)
		 *   -> java.sql 패키지에 존재함
		 * */
		...
		try {
			...
			// -> SELECT문 실행 메서드.결과로 java.sql.ResultSet 반환
			// 2) int Statement.executeUpdate(sql);
			// -> DML(INSERT, UPDATE, DELETE) 실행 메서드 결과로 int 반환(삽입, 수정, 삭제된 행의 개수)
			rs = stmt.executeQuery(sql);
			
			/* 6. 조회 결과가 담겨 있는 ResultSet을 커서(Cursor)를 
			   이용해 1행 씩 접근해 각 행의 작성된 컬럼값 얻어오기 */
			// rs.next() : 커서를 다음 행으로 이동 시킨 후 이동된 행에 값이 있으면 T, 없으면 F 반환
			//             맨 처음 호출 시 1행 부터 시작
			while(rs.next()) {
			...
			// -> 수행하지 않으면 DB와 연결된 Connection이 남아 있어서 다른 클라이언트가 
			//    추가적으로 연결되지 못하는 문제가 발생할 수 있다!!
			try {
