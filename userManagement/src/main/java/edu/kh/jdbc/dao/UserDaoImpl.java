...
public class UserDaoImpl implements UserDao{
	...
	// -> K, V가 모두 String 인 Map, 파일 입출력이 쉬움
	
	// 기본 생성자 생성
	public UserDaoImpl() {
		
		// 객체 생성 시 외부에 존재하는 sql.xml 파일을 읽어와 prop 에 저장
		try {
			String filePath = 
	...
	public int insertUser(Connection conn, User user) throws Exception {
		...
		try {
			...
			// 2. SQL 작성 -> Properties 를 이용해 외부 sql.xml 파일에서 읽어온 sql 이용
	...
	public List<User> search(Connection conn, String searchId) throws Exception {
		...
		// ArrayList 객체를 미리 생성하는 이유 == 조회된 결과를 추가해서 묶어서 반환하기 위해
		List<User> userList = new ArrayList<User>();
